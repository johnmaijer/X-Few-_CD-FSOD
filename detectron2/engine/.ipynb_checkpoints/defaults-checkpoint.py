...
""",
    ...
    parser.add_argument("--controller", action="store_true", help="add when FT-2heads-baseline")
    ...
    parser.add_argument(
        "--dist-url",
        default="auto", #"tcp://127.0.0.1:{}".format(port),
    ...
    def setup_custom_rpn(cfg):
        ...
        class CustomRPN(RPN):
            def __init__(self, cfg, input_shape):
                ...
                # 替换头部
                self.head = DropoutRPNHead(
                    in_channels=self.in_channels,
                    num_anchors=self.num_anchors,
                    dropout_prob=cfg.MODEL.RPN.DROPOUT_PROB
                )
        return CustomRPN
...
for _attr in ["model", "data_loader", "optimizer"]:
    setattr(
        ...
        property(
            # getter
            lambda self, x=_attr: getattr(self._trainer, x),# per_cls_input = torch.cat([intra_dist_emb,bg_dist_emb_c, bg_dist_emb_c], dim=1)TO
            # setter
