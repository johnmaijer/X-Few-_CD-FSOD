...
# from tools.build_cache_model import build_cache_model
...
def combined_focal_dice_loss(inputs, targets, gamma=0.5, reduction="mean",
                             bg_weight=0.2, num_classes=None, dice_weight=0.3, epsilon=1e-7):
    """
    组合Focal Loss和Dice Loss的多分类损失函数
    适用于单类别、小目标密集的遥感图像分类场景

    """
    # 计算Focal Loss部分
    fl = focal_loss(inputs, targets, gamma, reduction, bg_weight, num_classes)
    ...
    targets_one_hot = F.one_hot(targets, num_classes=2).float()  # (N, 2)

    # 计算每个类别的交集和并集
    intersection = torch.sum(probs * targets_one_hot, dim=0)  # (2,)
    union = torch.sum(probs, dim=0) + torch.sum(targets_one_hot, dim=0)

    # 按类别加权（背景类别特殊处理）
    dice_per_class = (2. * intersection + epsilon) / (union + epsilon)  # (2,)
    dice_loss = 1 - dice_per_class
    dice_loss = dice_loss * torch.tensor([1.0, bg_weight], device=inputs.device)

    # 按类别平均
    dice_loss = dice_loss.mean()

    # 组合两种损失
    total_loss = fl + dice_weight * dice_loss

    return total_loss


def binary_focal_loss(inputs, targets, gamma=2.0, alpha=0.25, bg_weight=0.2, reduction="mean"):
    """
    二分类 Focal Loss（适配形状 [N, 2] 的 logits）
    Args:
        inputs: 模型输出logits (N, 2)，[目标类分数, 背景类分数]
        targets: 标签 (N,)，0为背景，1为目标类
        gamma: 难样本聚焦参数（越大越关注难样本）
        alpha: 正类平衡权重（建议正样本少时设为0.25~0.5）
        bg_weight: 背景损失全局权重（通常<1以抑制背景）
        reduction: 聚合方式 ("mean", "sum"或None)
    """
    assert inputs.shape[1] == 2, "输入形状需为[N, 2]"

    # 分离目标类和背景类logits
    pos_logits = inputs[:, 0]  # 目标类logits (N,)
    neg_logits = inputs[:, 1]  # 背景类logits (N,)

    # 计算二元概率
    pos_probs = torch.sigmoid(pos_logits)  # 目标类概率 p
    neg_probs = torch.sigmoid(neg_logits)  # 背景类概率 1-p（若需要）

    # 根据标签选择有效logits
    pos_mask = (targets == 1)  # 正样本掩码
    valid_logits = torch.where(pos_mask, pos_logits, neg_logits)
    valid_probs = torch.where(pos_mask, pos_probs, neg_probs)
    ...
    # Focal调制因子
    p_t = torch.where(pos_mask, valid_probs, 1 - valid_probs)
    focal_term = (1 - p_t)  ** gamma

    # 类别权重（正类alpha，背景1-alpha）
    alpha_weight = torch.where(pos_mask, alpha, 1 - alpha)

    # 背景额外降权
    loss = ce_loss * focal_term * alpha_weight
    loss = torch.where(~pos_mask, loss * bg_weight, loss)
...
# class ProtoCenterAttention(nn.Module):
#     def __init__(self, input_size):
#         super(ProtoCenterAttention, self).__init__()
...
#         # Sum along the samples dimension
...
class ProtoCenterAttention(nn.Module):
    def __init__(self, input_size, num_heads=4, dropout=0.1):
        ...
        # 多头注意力的 QKV 线性变换
        self.qkv = nn.Linear(input_size, 3 * input_size)
    ...
    def forward(self, x):
        batch_size, num_samples, input_size = x.shape

        # 生成 QKV
        qkv = self.qkv(x).reshape(batch_size, num_samples, 3, self.num_heads, self.head_dim)
        ...
        # 加权聚合 Values
        weighted = torch.matmul(attn_scores, values)
        weighted = weighted.transpose(1, 2).reshape(batch_size, num_samples, input_size)

        # 投影层
        weighted = self.proj(weighted)
        weighted = self.proj_dropout(weighted)

        # 沿样本维度聚合（加权平均）
        aggregated = weighted.mean(dim=1)  # 或加权.sum(dim=1)

        # 全连接层
        output = self.fc(aggregated)

        # 返回输出和注意力分数（取第一个头的注意力分数）
        return output, attn_scores[:, 0, :, :].mean(dim=1)  # 示例：取第一个头的注意力分数


class InstReweight(nn.Module):

    def __init__(self, c_in, ratio=0.5):
        super(InstReweight, self).__init__()
        self.proto_center_attn = ProtoCenterAttention(c_in).cuda()
        self.ratio = ratio

    def forward(self, x):
        x_attn, attn_score = self.proto_center_attn(x)
        out = self.ratio * F.normalize(x_attn, dim=-1) + (1 - self.ratio) * F.normalize(x.mean(dim=1), dim=-1)
        out = F.normalize(out, dim=-1)
        return out, attn_score


class AttentionClassScoring(nn.Module):
    def __init__(self, feat_dim, num_classes, num_heads=4):
        ...
        # 将类别权重映射为 Query 向量
        self.class_to_query = nn.Linear(feat_dim, feat_dim)

        # 将 ROI 特征映射为 Key 和 Value 向量
        self.feature_to_key = nn.Linear(feat_dim, feat_dim)
        self.feature_to_value = nn.Linear(feat_dim, feat_dim)

        # 多头注意力机制（batch_first=True 表示输入形状为 (B, seq_len, feat_dim)）
        ...
        # 分类得分计算
        self.scorer = nn.Linear(feat_dim, 1)

    def forward(self, roi_features, class_weights):
        """
        Args:
            roi_features: (B, C, H*W)  # 输入特征（已展平）
            class_weights: (num_classes, C)  # 类别权重向量
        Returns:
            init_scores: (B, num_classes)  # 初始类别得分
        """
        B, C, num_patches = roi_features.shape

        class_weights = class_weights.to(roi_features.device)  # 新增此行

        # 调整特征形状: (B, C, H*W) -> (B, H*W, C)
        roi_flat = roi_features.permute(0, 2, 1)  # (B, num_patches, C)

        # 将类别权重扩展为 Batch 维度: (num_classes, C) -> (B, num_classes, C)
        class_queries = class_weights.unsqueeze(0).expand(B, -1, -1)  # (B, num_classes, C)

        # 生成 Query、Key、Value
        queries = self.class_to_query(class_queries)  # (B, num_classes, C)
        keys = self.feature_to_key(roi_flat)  # (B, num_patches, C)
        values = self.feature_to_value(roi_flat)  # (B, num_patches, C)

        # 交叉注意力计算: 类别作为 Query，特征作为 Key/Value
        ...
        # 计算得分
        scores = self.scorer(attn_output).squeeze(-1)  # (B, num_classes)
        return scores
...
class LinearClassifier(nn.Module):
    ...
    def __init__(self, dim, num_labels=1000, add_mlp=False, act_layer=nn.GELU, drop=0.):
...
class OpenSetDetectorWithExamples(nn.Module):
    ...
    def __init__(self,
        ...
        self.set_cd_vito = set_cd_vito
        self.train_test_overlap = train_test_overlap

        if controller:
            self.set_cd_vito = False
        ...
        print("CACHE_MODEL_VALUES path:", cache_model_values_file)  # 检查路径是否正确
        ...
        if not self.train_test_overlap and len(prototypes.shape) == 3:
            print("save fixed unseen test class proto")
            self.init_test_weight = F.normalize(prototypes.mean(dim=1)[torch.as_tensor(test_class_order)],
                                                dim=-1).cuda()
            index_dict = {value: index for index, value in enumerate(test_class_order)}
            self.train_in_test_index = [index_dict[value] for value in train_class_order]
        self.ndim = prototypes.shape[-1]

        if self.set_cd_vito and self.num_train_classes != 1:
            self.num_domain_prompter = int(self.num_train_classes * 2)
            self.domain_prompter = nn.Embedding(num_embeddings=self.num_domain_prompter, embedding_dim=self.ndim).cuda()
            self.domain_prompter_info_nce = InfoNCE()
            self.prototype_classifier = LinearClassifier(self.ndim, self.num_train_classes, add_mlp=False, drop=0.)
            self.prototype_ce = nn.CrossEntropyLoss()
            self.proto_info_nce = InfoNCE(temperature=2, negative_mode='paired')
        if self.set_cd_vito and self.num_train_classes == 1:
            # self.num_domain_prompter = int(self.num_train_classes * 2)
            self.num_domain_prompter = 10
            self.domain_prompter = nn.Embedding(num_embeddings=self.num_domain_prompter, embedding_dim=self.ndim).cuda()
            self.domain_prompter_info_nce = InfoNCE()
        ...
        if len(prototypes.shape) == 3:
            if self.set_cd_vito:
                self.k_shot = prototypes.shape[-2]
                self.class_weights = nn.Embedding(num_embeddings=self.num_train_classes * self.k_shot,
                                                  embedding_dim=self.ndim).cuda()
                self.class_weights.weight = nn.Parameter(
                    prototypes[torch.as_tensor(train_class_order)].view(-1, self.ndim).cuda().clone())
                self.train_class_weight = None
                self.register_buffer("test_class_weight",
                                     F.normalize(prototypes.mean(dim=1)[torch.as_tensor(test_class_order)], dim=-1))
                # print("类别特征向量：", prototypes.shape[-1])
                self.adapter = InstReweight(prototypes.shape[-1], ratio=attn_fuse_ratio)
            else:
        ...
        # Baseline: init fixed param after mean process
        if not self.set_cd_vito:
        ...
        if self.set_cd_vito:
            self.bg_tokens = nn.Embedding(num_embeddings=len(bg_protos), embedding_dim=self.ndim).cuda()
        ...
        if self.use_mask:
            ...
            if self.use_mask_feat_input:
                if self.use_mask_ms_feat:
                    self.mask_feat_compress = nn.ModuleList(
                        [nn.Conv2d(self.train_class_weight.shape[-1], feat_inp_dim, 1, 1, 0)
                         for _ in range(3)])
            ...
            self.mp_layers = nn.ModuleList([
                nn.Sequential(
                    nn.Conv2d(((self.Temb * 2 + feat_inp_dim) if i == 0 else hidden_dim) + i + layer_start_offset,
                              hidden_dim,
                              kernel_size=3, stride=1, padding=1),
                    nn.InstanceNorm2d(hidden_dim, affine=True) if self.use_mask_inst_norm else nn.BatchNorm2d(
                        hidden_dim),
                    nn.ReLU(),
    ...
    def update_test_buffer(self, new_values):
        if self.train_test_overlap:
            self.test_class_weight.copy_(new_values)
        else:
            self.init_test_weight[self.train_in_test_index] = new_values
            self.test_class_weight.copy_(self.init_test_weight)
    ...
    def mask_forward(self, features, boxes, class_labels, class_weights, gt_masks=None, feature_dict=None):
        ...
        if self.set_cd_vito:
            bg_roi_feats = roi_feats.transpose(-2, -1) @ self.bg_tokens.weight.t()  # N x K2 x back
        else:
        ...
        if self.set_cd_vito:
            fg_roi_feats = roi_feats.transpose(-2, -1) @ class_weights.weight.t()  # N x K2 x class
        else:
        ...
        if self.use_mask_feat_input:
            if self.use_mask_ms_feat:
                ...
                ms_feats = [self.mask_roi_align(feature_dict[k], boxes) for k in sorted(feature_dict.keys())] + [
                    roi_feats]
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if self.set_cd_vito:
            # print("输入的特征向量：", self.class_weights.weight.view(-1, self.k_shot, self.ndim))
            # print("输入的特征向量形状：", self.class_weights.weight.view(-1, self.k_shot, self.ndim).shape)
            self.train_class_weight, _ = self.adapter(self.class_weights.weight.view(-1, self.k_shot, self.ndim))
            self.update_test_buffer(self.train_class_weight)
        if self.training:
            ...
            with torch.no_grad():
                ...
                for proposals_per_image, targets_per_image in zip(boxes, gt_instances):
                    ...
                    if self.training or self.evaluation_shortcut:
                        ...
                        perm2 = torch.randperm(negative.numel())[:num_neg].to(
                            self.device)  # torch.randperm(negative.numel(), device=negative.device)[:num_neg]
        ...
        # %% #! Classification
        if self.training and (not self.only_train_mask):
            ...
            feats = roi_features.transpose(-2, -1) @ class_weights.T  # 计算ROI特征与类别原型的相似度
            ...
            if num_classes > 1:
                ...
                inter_dist_emb = other_classes.reshape(bs * num_active_classes, -1, self.roialign_size,
                                                       self.roialign_size)
            intra_feats = torch.gather(feats, 2, class_indices[:, None, :].repeat(1, spatial_size,
                                                                                  1)) if sample_class_enabled else feats
            intra_dist_emb = distance_embed(intra_feats.flatten(0, 1),
                                            num_pos_feats=self.Tpos_emb)  # (Nxspatial) x class x emb
            ...
            if num_classes > 1:
                ...
                per_cls_input = torch.cat([intra_dist_emb, bg_dist_emb_c, bg_dist_emb_c], dim=1)
            # (Nxclasses) x 1
            cls_logits = self.per_cls_cnn(per_cls_input)  # 分类得分
            ...
            if isinstance(bg_logits, list):
                ...
                # N x (classes + 1)   N x (topk + background)
                logits = torch.cat([cls_logits, bg_logits], dim=1)
                logits = logits / self.cls_temp
        elif not self.training:
            roi_features = roi_features.flatten(2)
            # print("ROI特征:", roi_features.shape)  # [1024, 1024, 49]
            bs, spatial_size = roi_features.shape[0], roi_features.shape[-1]

            # print("cache_keys形状:", self.cache_model_keys.shape)
            # print("cache_keys:", self.cache_model_keys)

            affinity = F.normalize(roi_features.flatten(2).mean(2), dim=1) @ self.cache_model_keys.to(self.device)
            # print("affinity:", affinity)
            beta, alpha = 1, 1
            cache_logits = ((-1) * (beta - beta * affinity)).exp() @ self.cache_model_values.to(self.device)
            cache_logits /= cache_logits.norm(dim=0, keepdim=True)
            # cache_logits = torch.sigmoid(cache_logits)
            # print("缓存分类分数：", cache_logits)
            # print(f"缓存分类分数形状：", cache_logits.shape)  # 直接使用PyTorch张量的shape属性

            # (N x spatial x emb) @ (emb x class) = N x spatial x class
            feats = roi_features.transpose(-2, -1) @ class_weights.T
            # print("加权后roi特征：", feats.shape)  # [1024, 49, 6]

            # sample topk classes  采用topk类别的部分
            class_topk = self.num_sample_class
            class_indices = None
            if class_topk < 0:
                class_topk = num_classes
            ...
            if sample_class_enabled:
                num_active_classes = class_topk
                # attention_scoring = AttentionClassScoring(feat_dim=roi_features.shape[1], num_classes=class_weights.shape[0]).to(self.device)
                # init_scores = attention_scoring(roi_features, class_weights)
                init_scores = F.normalize(roi_features.flatten(2).mean(2), dim=1) @ class_weights.T
                # print("init_score", init_scores.shape)
                topk_class_indices = torch.topk(init_scores, class_topk, dim=1).indices
            ...
            if isinstance(cls_logits, list):
                cls_logits = [v.reshape(bs, num_active_classes) for v in cls_logits]
                # cls_logits = [torch.sigmoid(cls_logits[i]) for i in range(len(cls_logits))]
            else:
                cls_logits = cls_logits.reshape(bs, num_active_classes)
                # cls_logits = torch.sigmoid(cls_logits)
            # print("分类分数cls_logits：", cls_logits)  # 1024个roi，每个取前topk个分类分数
            # for i, logit in enumerate(cls_logits):
            #     print(f"分类分数 {i} 形状：", logit.shape)  # 直接使用PyTorch张量的shape属性
            if isinstance(cls_logits, list):
                for i in range(len(cls_logits)):
                    cls_logits[i] += cache_logits * alpha
            else:
                cls_logits = cls_logits + cache_logits * alpha
            # print("总分类分数cls_logits：", cls_logits)
            ...
            if isinstance(bg_logits, list):
                ...
                # N x (classes + 1)   N x (topk + background)
                logits = torch.cat([cls_logits, bg_logits], dim=1)
                logits = logits / self.cls_temp  # (1024,topk+1)
        ...
        if (self.training and (not self.only_train_mask)) or (not self.training):
            ...
            if self.training:
                ...
                aug_rois, pred_roi_mask, gt_roi_mask, covered_flag = augment_rois(fg_proposals, matched_gt_boxes,
                                                                                  img_h=H, img_w=W,
                                                                                  pooler_size=self.reg_roialign_size,
                                                                                  min_expansion=0.4,
                ...
                reg_bs = len(rois)
                aug_rois, pred_roi_mask, _, _ = augment_rois(rois[:, 1:], None, img_h=H, img_w=W,
                                                             pooler_size=self.reg_roialign_size,
            ...
            if self.set_cd_vito:
                bg_aroi_feats = aroi_feats.transpose(-2, -1) @ self.bg_tokens.weight.t()
            else:
            ...
            if self.training:
                # N x emb x K x k
                bg_aroi_emb = bg_aroi_emb.permute(0, 2, 1).reshape(reg_bs, self.Temb, self.reg_roialign_size,
                ...
                fg_aroi_feats = torch.gather(fg_aroi_feats, 2, fg_class_labels[..., None, None].repeat(1, K2, 1))[:, :,
                                0]

                fg_aroi_emb = distance_embed(fg_aroi_feats,
                                             num_pos_feats=self.Tpos_emb)  # N x K2 x emb
                ...
                # (NxK2) x class x emb
                fg_aroi_dist_feats = torch.gather(fg_aroi_feats, 2, class_indices[:, None, :].repeat(1, K2,
                                                                                                     1)) if sample_class_enabled else fg_aroi_feats
            ...
            for i, (rp, rp_out) in enumerate([
                ...
                if self.training:
                    ...
                    loss_dict[f"aux_bce_loss_{i}"] = sigmoid_ce_loss(pred_mask_logits.flatten(1),
                                                                     gt_roi_mask.flatten(1), num_masks)
                    loss_dict[f"aux_dice_loss_{i}"] = dice_loss(pred_mask_logits.flatten(1), gt_roi_mask.flatten(1),
                                                                num_masks)
            ...
            fg_pred_deltas = pred_deltas = self.box2box_transform.get_deltas(
                fg_proposals if self.training else rois[:, None, 1:].repeat(1, num_active_classes, 1).flatten(0, 1),
                pred_abs_boxes)
        ...
        if self.training:
            ...
            if not self.only_train_mask:
                ...
                if isinstance(logits, list):
                    ...
                    for i, l in enumerate(logits):
                        ...
                        #     loss = focal_loss(l, class_labels, num_classes=num_active_classes, bg_weight=self.bg_cls_weight)
                        # else:
                        #     loss = combined_focal_dice_loss(l, class_labels, gamma=2.0,
                        #                                    bg_weight=0.1, reduction='mean', num_classes=1)
                        loss_dict[f'focal_loss_{i}'] = loss
                else:
                    _log_classification_stats(logits.detach(), class_labels)
                    loss = focal_loss(logits, class_labels, num_classes=num_active_classes,
                                          bg_weight=self.bg_cls_weight)
                    # if num_classes > 1:
                    #     loss = focal_loss(logits, class_labels, num_classes=num_active_classes,
                    #                       bg_weight=self.bg_cls_weight)
                    # else:
                    #     loss = binary_focal_loss(logits, class_labels, gamma=2.0, alpha=0.25, bg_weight=self.bg_cls_weight, reduction='mean')
            ...
            if self.set_cd_vito:
                if self.num_train_classes != 1:
                    class_prototypes = F.normalize(self.class_weights.weight.view(-1, self.k_shot, self.ndim).mean(dim=1),
                                                   dim=-1)
                    domain_prompter = F.normalize(self.domain_prompter.weight, dim=-1)
                    domain_prompter_contrasitive_loss = self.domain_prompter_info_nce(domain_prompter, domain_prompter)
                    loss_dict['domain_prompter_contrasitive_loss'] = domain_prompter_contrasitive_loss

                    random_indices = torch.randint(0, self.num_domain_prompter, (len(class_prototypes),)).cuda()
                    random_domain_prompter = domain_prompter[random_indices]
                    augmented_class_prototypes = class_prototypes + random_domain_prompter
                    prototype_classification_output = self.prototype_classifier(augmented_class_prototypes)
                    original_class_indices = torch.arange(0, len(class_prototypes)).cuda()
                    prototype_cls_loss = self.prototype_ce(prototype_classification_output, original_class_indices)
                    loss_dict['prototype_cls_loss'] = prototype_cls_loss
                    ...
                    for i in range(len(class_prototypes)):
                        rand_idx = torch.randint(0, self.num_domain_prompter, (1,))
                        enhanced_prototype = class_prototypes[i] + domain_prompter[rand_idx]
                        positive_keys.append(enhanced_prototype)
                        negative_samples = torch.cat([class_prototypes[:i], class_prototypes[i + 1:]], dim=0)
                        negative_samples = negative_samples + domain_prompter[rand_idx]
                        negative_keys.append(negative_samples)
                    positive_keys = torch.cat(positive_keys, dim=0)
                    negative_keys = torch.cat([x.unsqueeze(0) for x in negative_keys], dim=0)
                    rand_idx = torch.randint(0, self.num_domain_prompter, (len(class_prototypes),))
                    query = class_prototypes + domain_prompter[rand_idx]
                    proto_contrastive_loss = self.proto_info_nce(query, positive_keys, negative_keys)
                    loss_dict['proto_contrastive_loss'] = proto_contrastive_loss
#                 else:
#                     class_prototypes = F.normalize(
#                         self.class_weights.weight.view(-1, self.k_shot, self.ndim).mean(dim=1),
#                         dim=-1)
#                     domain_prompter = F.normalize(self.domain_prompter.weight, dim=-1)
#                     domain_prompter_contrasitive_loss = self.domain_prompter_info_nce(domain_prompter, domain_prompter)
#                     loss_dict['domain_prompter_contrasitive_loss'] = domain_prompter_contrasitive_loss

#                     bg_prototypes = F.normalize(self.bg_tokens.weight[:10], dim=-1)
#                     # 构造对比样本
#                     positive_keys = []
#                     negative_keys = []
#                     # 对每个背景原型生成对比样本
#                     for bg_idx in range(len(bg_prototypes)):
#                         # 正样本：目标类原型 + 随机域提示
#                         pos_rand_idx = torch.randint(0, self.num_domain_prompter, (1,))
#                         positive = class_prototypes + domain_prompter[pos_rand_idx]

#                         # 负样本：背景原型 + 相同域提示（增强对抗性）
#                         negative = bg_prototypes[bg_idx] + domain_prompter[pos_rand_idx]

#                         positive_keys.append(positive)
#                         negative_keys.append(negative)

#                     # 合并样本（保持与多类别一致的维度）
#                     positive_keys = torch.cat(positive_keys, dim=0)
#                     negative_keys = torch.stack(negative_keys)

#                     # 生成查询向量：目标类 + 随机域提示
#                     query_rand_idx = torch.randint(0, self.num_domain_prompter, (1,))
#                     query = class_prototypes + domain_prompter[query_rand_idx]  # [1, ndim]

#                     # 对比损失计算（适配单类别结构）
#                     proto_contrastive_loss = self.proto_info_nce(
#                         query.repeat(positive_keys.shape[0], 1),
#                         positive_keys,
#                         negative_keys
#                     )
#                     loss_dict['proto_contrastive_loss'] = proto_contrastive_loss * 2
            return loss_dict
        else:
