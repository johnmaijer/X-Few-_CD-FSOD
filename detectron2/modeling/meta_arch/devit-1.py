...
#         # 偏移量生成网络（输出 2*N 个偏移量，N=kernel_size^2）
...
#         # 此处简化实现，实际需使用双线性插值采样（参考Deformable ConvNet代码）
#         return x

# class ProtoCenterAttention(nn.Module):
#     def __init__(self, input_size, num_deform_groups=4, kernel_size=3):
#         super(ProtoCenterAttention, self).__init__()
...
#         # 注意力权重生成层
#         self.attn_fc = nn.Linear(input_size, 1)
...
#         # 将输入转换为3D格式（假设空间维度为 H=1, W=N）
#         x_3d = x.view(batch_size, num_samples, input_size).permute(0, 2, 1).unsqueeze(-1)  # [B, D, N, 1]

#         # 应用可变形卷积（处理空间关系）
#         deform_feat = self.deform_conv(x_3d)  # [B, D, N, 1]
#         deform_feat = deform_feat.squeeze(-1).permute(0, 2, 1)  # [B, N, D]

#         # 计算注意力权重
#         attn_weights = self.attn_fc(deform_feat).softmax(dim=1)  # [B, N, 1]

#         # 加权聚合
#         aggregated = (attn_weights * deform_feat).sum(dim=1)  # [B, D]
...
class OpenSetDetectorWithExamples(nn.Module):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if self.training and (not self.only_train_mask):
            ...
            bg_logits = self.bg_cnn(torch.cat([bg_cls_dist_emb, bg_dist_emb], dim=1))
            if isinstance(bg_logits, list):
                bg_logits = [torch.sigmoid(bg_logits[i]) for i in range(len(bg_logits))]
            else:
                bg_logits = torch.sigmoid(bg_logits)
            ...
            for i, logit in enumerate(logits):
                print(f"总分类分数 {i} 形状：", logit.shape)  # 直接使用PyTorch张量的shape属性
        else:
            if self.training:
